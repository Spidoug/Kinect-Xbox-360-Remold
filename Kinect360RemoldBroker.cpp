...
bool Validate1473RuntimeConfiguration(PhysicalNuiSession& session) {
    ...
    // libfreenect enables motor-via-audio only after the post-firmware audio
    // device exposes its full multi-interface runtime. For the 1473 that means
    // control/security/audio (MI_00/MI_01/MI_02), not a partial two-interface
    // 02BB enumeration.
