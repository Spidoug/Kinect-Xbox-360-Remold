...
The Kinect 1414 has one physical video engine on endpoint `0x81`; RGB and raw IR are mutually exclusive. The native IP runtime follows the virtual-camera RGB transport. If the **Surveillance** tab in `SynKinectStudio` switches endpoint `0x81` to IR while armed, the shared RGB frame stops advancing and the IP runtime reports the source as stale until RGB resumes. The service does not force a mode change because doing so would break the surveillance/scanner ownership rule.
