...
`02AD boot/WinUSB → Microsoft UACFirmware upload → 02BB MI_02/USB Audio → WASAPI → raw 4-channel fan-out → `Kinect360RemoldAudio` / `Kinect360RemoldAcoustic` → SynKinect Studio (Microphones + Acoustic Scanner tabs)`
...
AudioBridge enumerates active capture endpoints through MMDevice and accepts only the Kinect runtime whose `PKEY_Device_InstanceId` contains `VID_045E&PID_02BB` and `MI_02`. V1 has no friendly-name fallback. It accepts compatible 4-or-more-channel 16 kHz PCM16/24/32 or float32 shared-mode formats and converts the first four channels to the Remold S32LE ABI.
