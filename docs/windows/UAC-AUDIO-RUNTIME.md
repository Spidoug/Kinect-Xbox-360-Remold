...
For Windows, the builder validates the complete extracted image against its pinned SHA-256. UACFirmware 01.02.709.00 constructs its USB descriptors in ARM code at runtime and already emits `bInterval=4` for Audio ISO IN (`0x82`, synchronous isochronous, 524-byte maximum packet). This is an eight-microframe polling period, the maximum supported by the Windows high-speed isochronous scheduler. No executable firmware byte or independent `02AE` RGB/IR/Depth camera descriptor is modified.
