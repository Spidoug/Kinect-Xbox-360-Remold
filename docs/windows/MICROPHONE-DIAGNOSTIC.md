...
The AudioBridge uses overlapped `ConnectNamedPipe`. Its completion path must always pass a valid transferred-byte storage pointer to `GetOverlappedResult`; using a null pointer can make an otherwise valid asynchronous client connection fail before the protocol handshake. The driver keeps independent monitor and acoustic pipes, and each Processing client retries only its canonical endpoint.
