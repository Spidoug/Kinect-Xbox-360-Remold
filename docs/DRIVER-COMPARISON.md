...
| Four-mic runtime | firmware bootstrap + ALSA | supported with firmware path | outside camera driver | legacy stack-specific |
| Multi-application ownership | one Remold owner + fan-out IPC | normally application/library ownership | V4L2 semantics | legacy client/server limitations vary |
| Hot-plug service policy | udev + systemd + retry loops | application responsibility plus udev setup | kernel/V4L2 device lifecycle | legacy middleware lifecycle |
...
| Maturity | experimental/new | mature community project | mature kernel component | historical/legacy |
