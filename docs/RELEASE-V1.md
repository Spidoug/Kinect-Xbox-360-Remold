...
V1.0 is the only supported baseline in this source tree. There is no migration layer, decoded-camera wire ABI, alternate protocol parser or duplicate native payload. ScannerPort is RAW-only on Windows and Linux: RGB is GRBG8 Bayer, IR is packed 10-bit and Depth is packed 11-bit shift data. Any client sending or expecting the removed decoded formats is rejected by the V1 contract.
