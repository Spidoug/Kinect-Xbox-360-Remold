...
Generated JARs, platform JOGL/GlueGen libraries, launchers copied from these templates and runtime data are created under `applications/binaries/<platform>/` by the build. That directory is generated by the build and is not committed to the repository.
...
On Linux, the builder also creates an embedded Java runtime under `binaries/linux-x64/java` using `jdeps`/`jlink`; a system-wide Java installation is not required to run the generated Studio.
