...
function Recover-1473IncompleteAudioRuntime {
    # Functional recovery only: a 1473 that exposes a post-UAC control
    # interface (02BB/02C3 MI_00) but neither USB-Audio MI_02 nor 02AD boot is
    # in a partial composite state. Recover only the UAC audio composite. Never
    # restart/rebind the 02C2 parent hub here, and never make this recovery a
    # prerequisite for camera/control installation.
    ...
    $parentId=($controlId -replace '&MI_00$','')
    $parent=Test-PresentHardwareIdNative $parentId
    if(!$parent.Present){
        Write-Host ("Kinect 1473 audio composite: partial child topology detected, but parent {0} is not addressable for a targeted restart." -f $parentId) -ForegroundColor Yellow
        return $false
    }

    Write-Host ("Kinect 1473 audio composite: MI_00 is present without 02AD/MI_02; rebuilding only {0}..." -f $parentId) -ForegroundColor DarkCyan
    [void](Stop-ServiceBounded $Product.Services.AudioBridge 4000 -ForceProcess -Quiet)
    [void](Stop-ServiceBounded $Product.Services.Broker 4000 -ForceProcess -Quiet)

    $restart=Invoke-Native $Setup @('restart',$parentId)
    if(!$Simple){Show-Native $restart}
    Rescan
    [void](Configure-AudioBridgeRuntime)
    [void](Start-ServiceBounded $Product.Services.Broker 5000 -Quiet)
    if($restart.ExitCode -eq 0 -and (Wait-1473AudioRuntime 20)){
        $control=Test-StartedWinUsb $controlId
        $script:Motor1473PnpReady=$control.Ready
        Update-MotorState
        Write-Host ("Kinect 1473 audio composite: topology returned after targeted {0} restart." -f $parentId) -ForegroundColor Green
        return $true
    }

    # One stronger retry remains scoped to the current UAC composite. This is deliberately not a
    # 02C2 hub cycle, so camera identity/port ownership is preserved.
    [void](Stop-ServiceBounded $Product.Services.AudioBridge 4000 -ForceProcess -Quiet)
    [void](Stop-ServiceBounded $Product.Services.Broker 4000 -ForceProcess -Quiet)
    $cycle=Invoke-Native $Setup @('cycle',$parentId)
    if(!$Simple){Show-Native $cycle}
    Rescan
    [void](Configure-AudioBridgeRuntime)
    [void](Start-ServiceBounded $Product.Services.Broker 5000 -Quiet)
    if($cycle.ExitCode -eq 0 -and (Wait-1473AudioRuntime 24)){
        $control=Test-StartedWinUsb $controlId
        $script:Motor1473PnpReady=$control.Ready
        Update-MotorState
        Write-Host ("Kinect 1473 audio composite: topology returned after one targeted {0} cycle." -f $parentId) -ForegroundColor Green
        return $true
    }
    ...
    $script:Motor1473PnpReady=$control.Ready
    Update-MotorState
    Write-Host 'Kinect 1473 audio composite: 02AD/MI_02 did not return; audio remains PENDING while camera/control continue independently.' -ForegroundColor Yellow
    return $false
}
