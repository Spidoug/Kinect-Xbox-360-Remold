...
class CameraHub {
 public:
  CameraHub() {
    rgb_.data.resize(scanner::kNv12PayloadBytes);
    ir_.data.resize(scanner::kGray16PayloadBytes);
    depth_.data.resize(scanner::kDepthPayloadBytes);
  }
  ...
  bool wait_next(uint32_t mask, uint64_t& rgb_seq, uint64_t& ir_seq, uint64_t& depth_seq,
    ...
    cv_.wait_for(lock, std::chrono::milliseconds(1000), [&] {
      return !run || !online_.load() ||
             ((mask & scanner::StreamRgb) && rgb_.seq != rgb_seq) ||
             ((mask & scanner::StreamInfrared) && ir_.seq != ir_seq) ||
             ((mask & scanner::StreamDepth) && depth_.seq != depth_seq);
    });
    if (!run || !online_) return false;

    Frame* frame = nullptr;
    scanner::StreamMode mode = scanner::StreamMode::Depth;
    scanner::PixelFormat pixel_format = scanner::PixelFormat::DepthMm16;
    uint32_t flags = 0;

    if ((mask & scanner::StreamRgb) && rgb_.seq != rgb_seq) {
      frame = &rgb_;
      rgb_seq = frame->seq;
      mode = scanner::StreamMode::Rgb;
      pixel_format = scanner::PixelFormat::Nv12;
    } else if ((mask & scanner::StreamInfrared) && ir_.seq != ir_seq) {
      frame = &ir_;
      ir_seq = frame->seq;
      mode = scanner::StreamMode::Infrared;
      pixel_format = scanner::PixelFormat::Gray16;
    } else if ((mask & scanner::StreamDepth) && depth_.seq != depth_seq) {
      frame = &depth_;
      depth_seq = frame->seq;
      mode = scanner::StreamMode::Depth;
      pixel_format = scanner::PixelFormat::DepthMm16;
      if (frame->calibrated) flags |= scanner::kFlagDeviceCalibrated;
    } else {
      return true;
    }
    ...
    header = {};
    header.mode = mode;
    header.pixelFormat = pixel_format;
    header.payloadBytes = static_cast<uint32_t>(frame->data.size());
    header.flags = flags;
    header.frameNumber = frame->seq;
    header.tickMs = frame->tick;
    output = frame->data;
    return true;
  }
 ...
 private:
  ...
  void on_rgb(const uint8_t* data, std::size_t bytes, uint32_t) {
    ...
    rgb_to_nv12(data, rgb_.data);
    rgb_.tick = unixio::monotonic_ms();
    rgb_.calibrated = false;
    ++rgb_.seq;
    cv_.notify_all();
  }
  ...
  void on_ir(const uint16_t* data, std::size_t pixels, uint32_t) {
    ...
    std::lock_guard<std::mutex> lock(frame_mu_);
    std::memcpy(ir_.data.data(), data, scanner::kGray16PayloadBytes);
    ir_.tick = unixio::monotonic_ms();
    ir_.calibrated = false;
    ++ir_.seq;
    cv_.notify_all();
  }
  ...
  void on_depth(const uint16_t* data, std::size_t pixels, uint32_t, bool calibrated) {
    if (pixels < static_cast<std::size_t>(scanner::kWidth) * scanner::kHeight) return;
    std::lock_guard<std::mutex> lock(frame_mu_);
    std::memcpy(depth_.data.data(), data, scanner::kDepthPayloadBytes);
    depth_.tick = unixio::monotonic_ms();
    depth_.calibrated = calibrated;
    ++depth_.seq;
    cv_.notify_all();
  }
