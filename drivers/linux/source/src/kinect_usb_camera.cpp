/* SynKinect native Kinect Xbox 360 USB camera transport. */
...
constexpr int kIsoTransferPacketBytes = 1920;  // Kinect video isochronous transfer slot
 ...
 public:
  ...
  void process(const uint8_t* packet, int length) {
    ...
    if (!synced_) {
      ...
      // Fixed-stream recovery tolerates up to five
      // missing packets and keep the frame cadence.  Zero filling is preferable
      // to reusing stale pixels from the previous frame.
      const uint8_t lost = static_cast<uint8_t>(header.seq - expected_seq_);
      if (lost == 0 || lost > kMaxRecoverablePacketLoss ||
          packet_index_ + lost >= packets_per_frame_) {
        reset();
        if (header.flag != sof) return;
        start_frame(header.seq);
      } else {
        for (uint8_t i = 0; i < lost; ++i) {
          const std::size_t missing = packet_index_ == packets_per_frame_ - 1u
              ? last_packet_bytes_ : packet_payload_;
          std::memset(frame_.data() + packet_index_ * packet_payload_, 0, missing);
          ++packet_index_;
        }
        expected_seq_ = header.seq;
      }
    }
 ...
 public:
  ...
  int start_video(VideoMode mode) {
    ...
    if (mode == VideoMode::Rgb || mode == VideoMode::RgbHighQuality) {
      // Kinect camera protocol: Bayer format=0, VGA resolution=1 at 30 fps,
