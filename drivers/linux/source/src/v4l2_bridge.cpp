...
int main() {
  ...
  while (run) {
    ...
    bool client_active = !event_supported;  // compatibility fallback for old loopback releases
    int scanner_fd = -1;
    ...
    while (run) {
      ...
      pollfd pfd[2]{};
      pfd[0].fd = out;
      pfd[0].events = event_supported ? POLLPRI : 0;
      pfd[1].fd = scanner_fd;
      ...
      if (event_supported && (pfd[0].revents & POLLPRI)) {
