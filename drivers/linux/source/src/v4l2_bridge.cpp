...
int main() {
  ...
  while (run) {
    ...
    if (!event_supported) {
      std::fprintf(stderr, "v4l2: V1 requires v4l2loopback >= 0.15.0 client-usage events\n");
      ::close(out);
      unixio::retry_sleep(1000);
      continue;
    }
    ...
    while (run) {
      ...
      if (header.mode != scanner::StreamMode::Rgb ||
          ...
          header.payloadBytes != scanner::kRgbRawPayloadBytes) continue;
      rawsensor::bayer_grbg_to_rgb24(payload.data(), scanner::kWidth, scanner::kHeight, rgb);
