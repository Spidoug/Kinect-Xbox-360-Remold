...
The runtime accepts audio only after the active `02BB/02C3` USB descriptor matches the canonical V1 microphone profile: ISO IN `0x82`, synchronous data, 524-byte maximum packet and `bInterval=4`. A mismatched RAM-resident runtime is reported instead of being silently captured; power-cycle the sensor once so the `02AD` firmware loader can restore the V1 profile.
