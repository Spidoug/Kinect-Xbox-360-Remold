...
for arg in "$@"; do
  case "$arg" in
    --no-deps) NO_DEPS=1 ;;
    *) echo "Unknown option: $arg" >&2; echo "Usage: sudo bash $0 [--no-deps]" >&2; exit 2 ;;
  esac
done
...
install_deps(){
  ...
  if command -v apt-get >/dev/null; then
    apt-get update
    DEBIAN_FRONTEND=noninteractive apt-get install -y build-essential cmake pkg-config libusb-1.0-0-dev libasound2-dev libjpeg-dev v4l2loopback-dkms
  elif command -v dnf >/dev/null; then
    dnf install -y gcc-c++ cmake pkgconf-pkg-config libusb1-devel alsa-lib-devel libjpeg-turbo-devel v4l2loopback
  elif command -v pacman >/dev/null; then
    pacman -S --needed --noconfirm base-devel cmake pkgconf libusb alsa-lib libjpeg-turbo v4l2loopback-dkms
  else
    echo 'Unsupported package manager. Install CMake, libusb, ALSA, libjpeg and v4l2loopback manually, then use --no-deps.' >&2
    exit 2
  fi
}
...
bash "$ROOT/scripts/fetch-uac-firmware.sh" /usr/share/kinect360-remold/UACFirmware || \
  echo 'Current V1 UACFirmware was not supplied; camera/depth remains available and audio will retry when firmware is provided.' >&2
