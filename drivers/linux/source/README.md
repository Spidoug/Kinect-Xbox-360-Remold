...
The UAC firmware is not committed to this repository. `fetch-uac-firmware.sh` downloads the same pinned Microsoft Kinect SDK Beta 2 MSI used by the Windows build, validates its MD5 and extracts `UACFirmware` with `msiextract`.
