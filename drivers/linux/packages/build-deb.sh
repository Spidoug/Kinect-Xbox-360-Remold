...
ARCH="${1:-amd64}"
[[ "$ARCH" == amd64 ]] || { echo "Only amd64/x86_64 prebuilt payload is currently available." >&2; exit 2; }
VERSION="${REMOLD_DEB_VERSION:-1.0.0-1}"
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT
PKG="$WORK/pkg"
BIN="$ROOT/binaries/x86_64"
SRC="$ROOT/source"
...
install -m 0755 "$BIN/bin/kinect360-remoldctl" "$PKG/usr/bin/"
for file in "$BIN"/libexec/kinect360-remold/*; do install -m 0755 "$file" "$PKG/usr/libexec/kinect360-remold/"; done
...
Depends: libc6, libstdc++6, libusb-1.0-0, libasound2t64 | libasound2, libjpeg62-turbo, systemd, udev, kmod, curl, 7zip | p7zip-full, v4l2loopback-dkms
...
case "${1:-configure}" in
  configure)
    ...
    if [ -f "$CONF" ]; then
      if grep -q '^ip.password=$' "$CONF"; then
    ...
    if [ ! -f /usr/share/kinect360-remold/UACFirmware ]; then
      if ! /usr/share/kinect360-remold/fetch-uac-firmware.sh /usr/share/kinect360-remold/UACFirmware; then
        echo 'Kinect UAC firmware could not be downloaded during package setup; camera/depth remains installed, audio will retry after firmware is provided.' >&2
      fi
    fi
    ...
    systemctl enable kinect360-remold.target >/dev/null 2>&1 || true
    systemctl restart kinect360-remold.target >/dev/null 2>&1 || true
    ;;
esac
exit 0
POSTINST
...
OUT="$ROOT/packages/output/kinect360-remold_${VERSION}_${ARCH}.deb"
