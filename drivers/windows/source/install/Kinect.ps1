...
function Open-Studio{
    ...
    try{
        Start-Process -FilePath $studio -WorkingDirectory (Split-Path -Parent $studio) | Out-Null
        Write-Host ("SynKinect Studio opened: {0}" -f $studio) -ForegroundColor Green
    }catch{
        Write-Host 'Could not open SynKinect Studio.' -ForegroundColor Red
...
while($true){
    ...
    Write-Host '10 Open SynKinect Studio (Windows)'
    Write-Host '0  Exit'
    Write-Host ''
    ...
    switch($choice){
        ...
        '10'{Header;Open-Studio;Start-Sleep -Milliseconds 500}
        '0'{return}
