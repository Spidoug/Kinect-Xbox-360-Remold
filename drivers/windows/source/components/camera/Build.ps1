...
try {
    ...
    & $Tools.MSBuild $bridgeProj /m /t:Rebuild /p:Configuration=$Configuration /p:Platform=x64 /p:RemoldPlatformToolset=$($Tools.PlatformToolset) /p:PlatformToolset=$($Tools.PlatformToolset) /p:WindowsTargetPlatformVersion=$($Tools.WdkVersion)
    ...
    & (Join-Path $Root 'source\virtual-camera\Prepare-VirtualCamera.ps1') -SampleRoot $sample `
        ...
        -ActiveLeaseMs $Product.VirtualCameraActiveLeaseMs `
