...
public:
    ...
    void Publish(ScannerPort::StreamMode mode, ScannerPort::PixelFormat pixelFormat,
...

        {
            ...
            // FIFO transport preserves capture order. The Processing client has its
            // own bounded queues, so short reconstruction/export stalls no longer
            // overwrite the previous depth frame at the native bridge boundary.
            constexpr size_t kMaxQueuedFrames = 96;
            while (m_queue.size() >= kMaxQueuedFrames) m_queue.pop_front();
