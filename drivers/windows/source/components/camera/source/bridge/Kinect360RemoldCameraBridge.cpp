...
// Eight transfers keep 256 ISO packets continuously scheduled on Windows.
constexpr ULONG kIsoPacketsPerTransfer = 32;
constexpr ULONG kIsoQueueDepth = 8;
...
public:
    ...
    HRESULT Open() {
        ...
        m_header->online = 0;
        // Software consumers have no physical-mode request fields in the V1 protocol.
...
public:
    ...
    bool Push(const uint8_t* packet, ULONG length, std::vector<uint8_t>& completed,
        ...
        if (h.sequence != m_expectedSequence) {
            // Keep a fixed-size stream synchronized when at most five
            // isochronous packets are lost.  The 1473 is noticeably more likely
            // to expose an occasional missed microframe on busy USB 2 hosts.  A
            // hard reset here made RGB appear to freeze and made the longer IR
            // and Depth frames almost impossible to complete.
            ...
            if (lost == 0 || lost > kMaxRecoverablePacketLoss ||
                ...
                for (uint8_t i = 0; i < lost; ++i) {
                    ...
                    if (m_offset + missingBytes > m_raw.size()) { ResetSync(); return false; }
...
public:
    ...
    HRESULT Open(const std::wstring& devicePath) {
        ...
        if (m_is1473) {
            ...
            for (unsigned attempt = 0; attempt < 20 && g_run.load(); ++attempt) {
                if (PrepareCameraControl()) { prepared = true; break; }
                Sleep(250);
            }
            if (!prepared)
                Log(L"Kinect 1473 keep-alive was unavailable after 5 seconds; continuing camera with recovery enabled.");
        }
    ...
    HRESULT Run(SharedFramePublisher& publisher, ScannerPortServer& scannerPort,
        ...
        // Kinect is closed and reopened from a fresh handle.
...
private:
    ...
    void DetectHardwareRevision() {
        ...
        // 02B0 motor function. This revision distinction selects the 1473 profile.
    ...
    HRESULT ConfigureVideoModeRegisters(ScannerPort::StreamMode mode) {
        ...
        // first, preserving the camera firmware start/stop lifecycle.
        if (mode == ScannerPort::StreamMode::Rgb) {
    ...
    HRESULT StreamVideo(SharedFramePublisher& publisher,
        ...
        // Isochronous reads are armed before register 0x05 starts
        ...
        while (SUCCEEDED(result) && SessionRunning()) {
            ...
            if (wait == WAIT_TIMEOUT) {
                if (++timeouts == kMaxIsoTimeouts)
                    Log(L"Video ISO queue has not completed for 3 seconds; keeping pending transfers alive.");
                continue;
            }
            timeouts = 0;
    ...
    HRESULT StreamDepthSession(ScannerPortServer& scannerPort, FrameConversionWorker& depthWorker) {
        ...
        // Arm USB first, then enable register 0x06. Once
        ...
        while (SUCCEEDED(result) && SessionRunning()) {
            ...
            if (wait == WAIT_TIMEOUT) {
                ...
                if (wantsDepth) {
                    if (++timeouts == kMaxIsoTimeouts)
                        Log(L"Depth ISO queue has not completed for 3 seconds; keeping pending transfers alive.");
                } else {
                    timeouts = 0;
                }
                continue;
            ...
            if (!WinUsb_GetOverlappedResult(m_usb, &slot.ov, &ignored, FALSE)) {
                ...
                if (e == ERROR_OPERATION_ABORTED && !SessionRunning()) break;
                wchar_t message[160]{};
                swprintf_s(message, L"Depth ISO completion error %lu on slot %lu; resubmitting.", e, next);
                Log(message);
                if (!queueRead(next, TRUE)) { result = HrLastError(); break; }
                next = (next + 1) % kIsoQueueDepth;
                continue;
