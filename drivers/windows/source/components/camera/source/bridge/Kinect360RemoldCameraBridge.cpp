...
public:
    ...
    HRESULT Open(const std::wstring& devicePath) {
        ...
        // The 1473 camera is only stable after the post-firmware 02BB control
        // runtime is complete and its one-shot LED keep-alive has replied. Do
        // not start endpoint 0x81 on a partial MI_00/MI_01 enumeration: that is
        // the reconnect/flicker loop seen when MI_02 never finished appearing.
        if (m_is1473) {
            bool prepared = false;
            for (int attempt = 0; attempt < 12 && g_run.load(); ++attempt) {
                if (PrepareCameraControl()) { prepared = true; break; }
                Sleep(500);
            }
            if (!prepared) {
                Log(L"Kinect 1473 runtime control/audio topology is not ready; deferring camera stream instead of opening an unstable USB session.");
                Close();
