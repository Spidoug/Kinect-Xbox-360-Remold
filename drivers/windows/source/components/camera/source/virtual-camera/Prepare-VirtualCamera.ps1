...
param(
    ...
    [Parameter(Mandatory=$true)][int]$ActivityHeartbeatMs,
    [Parameter(Mandatory=$true)][int]$ActivityLeaseMs,
...
$sourceBlock=@'
        ...
        // Standard Windows camera formats from the Kinect RGB sensor. NV12 is
        // the native shared-memory transport, YUY2 covers broad video-capture
        // application support, and RGB32 provides a direct RGB format for applications
        // that request an RGB camera surface.
        ...
        RETURN_IF_FAILED(MFCreateMediaType(&spMediaType));
        spMediaType->SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Video);
        spMediaType->SetGUID(MF_MT_SUBTYPE, MFVideoFormat_YUY2);
        spMediaType->SetUINT32(MF_MT_INTERLACE_MODE, MFVideoInterlace_Progressive);
        spMediaType->SetUINT32(MF_MT_ALL_SAMPLES_INDEPENDENT, TRUE);
        MFSetAttributeSize(spMediaType.get(), MF_MT_FRAME_SIZE, NUM_IMAGE_COLS, NUM_IMAGE_ROWS);
        MFSetAttributeRatio(spMediaType.get(), MF_MT_FRAME_RATE, 30, 1);
        bitrate = (uint32_t)(NUM_IMAGE_COLS * NUM_IMAGE_ROWS * 2 * 8 * 30);
        spMediaType->SetUINT32(MF_MT_AVG_BITRATE, bitrate);
        MFSetAttributeRatio(spMediaType.get(), MF_MT_PIXEL_ASPECT_RATIO, 1, 1);
        ...
        spMediaType->SetUINT32(MF_MT_AVG_BITRATE, bitrate);
        MFSetAttributeRatio(spMediaType.get(), MF_MT_PIXEL_ASPECT_RATIO, 1, 1);
        mediaTypeList[2] = spMediaType.detach();
...
Require ($stream -match 'const uint32_t NUM_MEDIATYPES = 3;') 'Three standard camera media types were not inserted.'
...
foreach($pair in @(
    ...
    [pscustomobject]@{Token='__REMOLD_ACTIVITY_HEARTBEAT_MS__';Value=[string]$ActivityHeartbeatMs},
    [pscustomobject]@{Token='__REMOLD_ACTIVITY_LEASE_MS__';Value=[string]$ActivityLeaseMs},
...
# The upstream vcxproj currently contains both 3.x and older 2.x imports, and
