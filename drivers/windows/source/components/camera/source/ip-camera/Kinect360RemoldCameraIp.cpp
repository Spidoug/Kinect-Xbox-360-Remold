...
constexpr DWORD kActivityHeartbeatMs = 500;
...
bool LoadConfig(Config& cfg, std::string& error) {
    ...
    if (file == INVALID_HANDLE_VALUE) { error = "camera-ip.ini not found; run installer/repair"; return false; }
    LARGE_INTEGER size{};
...
public:
    ...
    bool Open() {
        ...
        if (header_->magic != Frame::kMagic || header_->version != Frame::kVersion ||
            header_->width != Frame::kWidth || header_->height != Frame::kHeight ||
            header_->fourcc != Frame::kNv12Fourcc || header_->frameBytes != Frame::kNv12Bytes) {
            Close(); return false;
        }
        return true;
    }
    bool Read(std::vector<uint8_t>& nv12, uint64_t& frameNumber, uint64_t& tickMs, bool& online) {
        ...
        for (int attempt=0;attempt<4;++attempt) {
            const LONG before = header_->sequence;
            ...
            if (meta.bytes != Frame::kNv12Bytes) { online = onlineRaw != 0; return false; }
            nv12.resize(Frame::kNv12Bytes);
            std::memcpy(nv12.data(), Frame::SlotAddress(base_, slot), Frame::kNv12Bytes);
            MemoryBarrier();
            const LONG after = header_->sequence;
...
bool NotifyBrokerIpCameraActivity() {
    if (!WaitNamedPipeW(Control::kPipeName, 50)) return false;
    ...
    const bool ok = WriteFile(pipe, &request, sizeof(request), &written, nullptr) &&
                    ...
                    read == sizeof(reply) &&
                    reply.magic == Control::kMagic &&
                    reply.version == Control::kVersion &&
                    SUCCEEDED(static_cast<HRESULT>(reply.result));
    CloseHandle(pipe);
    return ok;
}

void CameraActivityLoop() {
    while (g_running.load()) {
        if (g_authenticatedClients.load(std::memory_order_acquire) != 0) {
            (void)NotifyBrokerIpCameraActivity();
        }
        if (WaitForSingleObject(g_stopEvent, kActivityHeartbeatMs) == WAIT_OBJECT_0) break;
    }
}
...
DWORD RunRuntime() {
    ...
    std::thread activity(CameraActivityLoop);
    WaitForSingleObject(g_stopEvent,INFINITE);
    StopRuntime();
    if(server.joinable())server.join();
    if(activity.joinable())activity.join();
