...
public:
    ...
    void Write(bool force = false) {
        ...
        text << "uac_profile=remold-v1-1414-compatible\r\n";
        text << "uac_iso_in_endpoint=0x82\r\n";
        text << "uac_iso_in_attributes=0x0d\r\n";
        text << "uac_iso_in_max_packet=524\r\n";
        text << "uac_iso_in_binterval=4\r\n";
        text << "uac_channels=4\r\n";
...
void CaptureLoop(RawAudioBus* port) {
    ...
    while (gRun.load()) {
        ...
        gDiagnostics.SetStage("uac-endpoint-not-found", ERROR_NOT_FOUND,
                              "no active V1 UAC capture endpoint is available; the 02AD worker will load the pinned 01.02.709.00 profile (EP 0x82, 524 bytes, bInterval 4). If a 1473 is already running another RAM audio image, power-cycle the sensor once so it returns to 02AD.");
        Sleep(kAudioRetryMs);
    }
