...
// Canonical V1 microphone transport after UAC firmware launch.  The 1473 uses
// the same host-visible audio stream contract as the supported 1414 path while
// retaining its own MI_00 control topology for motor/LED/accelerometer.
// Windows high-speed isochronous scheduling supports bInterval 1..4; keep the
// microphone stream at the 8-microframe service interval represented by 4.
inline constexpr UCHAR kRuntimeIsoInEndpoint = 0x82;
inline constexpr UCHAR kRuntimeIsoInAttributes = 0x0D; // ISO + synchronous + data
inline constexpr USHORT kRuntimeIsoInMaxPacketBytes = 524;
inline constexpr UCHAR kRuntimeIsoInBInterval = 4;
inline constexpr uint32_t kRuntimeSampleRate = 16000u;
inline constexpr uint32_t kRuntimeChannels = 4u;
inline constexpr uint32_t kRuntimeBitsPerSample = 32u;
