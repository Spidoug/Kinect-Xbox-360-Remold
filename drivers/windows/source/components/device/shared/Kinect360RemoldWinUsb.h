...
// transport. After Microsoft UACFirmware launches, the 02BB/02C3 MI_02 runtime is
...
public:
    ...
    BOOL SetPipePolicy(Handle handle, UCHAR pipe, ULONG policyType, ULONG valueLength, PVOID value) const {
        return m_setPipePolicy ? m_setPipePolicy(handle, pipe, policyType, valueLength, value) : FailBool();
    }
...
private:
    ...
    using SetPipePolicyFn = BOOL (WINAPI*)(WINUSB_INTERFACE_HANDLE, UCHAR, ULONG, ULONG, PVOID);
...
#define REMOLD_WINUSB_REQUIRED(member, symbol) ok = ResolveRequired(member, symbol) && ok
        ...
        REMOLD_WINUSB_REQUIRED(m_setPipePolicy, "WinUsb_SetPipePolicy");
...
#undef REMOLD_WINUSB_REQUIRED
    ...
    SetPipePolicyFn m_setPipePolicy = nullptr;
