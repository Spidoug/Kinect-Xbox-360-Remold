...
try{
    ...
    # there is no literal endpoint descriptor to byte-patch safely.  Pin the
    # complete firmware image and also pin the host-visible endpoint policy so a
    # future product edit cannot silently reintroduce the 02AD bInterval=5 path.
    $targetEndpoint=[int]$Product.KinectUacIsoInEndpoint
    $targetAttributes=[int]$Product.KinectUacIsoInAttributes
    $targetPacket=[int]$Product.KinectUacIsoInMaxPacketBytes
    $targetInterval=[int]$Product.KinectUacIsoInBInterval
    $targetRate=[int]$Product.KinectUacSampleRate
    $targetChannels=[int]$Product.KinectUacChannels
    $targetBits=[int]$Product.KinectUacBitsPerSample
    if($targetEndpoint -ne 0x82 -or $targetAttributes -ne 0x0D -or $targetPacket -ne 524 -or
       $targetInterval -ne 4 -or $targetRate -ne 16000 -or $targetChannels -ne 4 -or $targetBits -ne 32){
        throw ("Unsupported Kinect V1 UAC endpoint policy: ep=0x{0:X2} attr=0x{1:X2} packet={2} interval={3} rate={4} channels={5} bits={6}" -f $targetEndpoint,$targetAttributes,$targetPacket,$targetInterval,$targetRate,$targetChannels,$targetBits)
    }
    ...
    Write-Host ("UACFirmware {0} validation: PASS (SHA256={1}, bytes={2}, load=0x00080000, entry=0x00080030; UAC profile=EP0x82/ISO-SYNC/524/BInterval4/4ch/16k/S32; models=1414,1473)" -f $expectedFirmwareVersion,$firmwareSha,$fw.Length) -ForegroundColor Green
