...
function Test-PythonExecutable([string]$candidate) {
    ...
    try {
        $output = & $candidate -c "import sys; print(sys.version_info[0]); print(sys.executable)" 2>$null
        if ($LASTEXITCODE -ne 0) { return $false }
        if (!$output -or $output.Count -lt 1 -or $output[0].Trim() -ne '3') { return $false }
        return $true
    } catch { return $false }
}

function Find-Python {
    # Prefer the Python launcher, but verify it is actually executable.  The
    # WindowsApps python.exe alias may exist even when Python is not installed.
    $cmd = Get-Command py.exe -ErrorAction SilentlyContinue
    if ($cmd -and $cmd.Source -and (Test-PythonExecutable $cmd.Source)) { return $cmd.Source }

    $candidates = @()
    if ($env:LOCALAPPDATA) {
        $pythonRoot = Join-Path $env:LOCALAPPDATA 'Programs\Python'
        if (Test-Path -LiteralPath $pythonRoot) {
            $candidates += Get-ChildItem -LiteralPath $pythonRoot -Recurse -File -Filter python.exe -ErrorAction SilentlyContinue |
                Sort-Object FullName -Descending | ForEach-Object FullName
        }
    }
    foreach ($base in @($env:ProgramFiles, ${env:ProgramFiles(x86)})) {
        if (!$base) { continue }
        # Do not hard-code Python minor versions; enumerate normal python.org
        # installation folders so future Python releases keep working.
        $candidates += Get-ChildItem -LiteralPath $base -Directory -Filter 'Python3*' -ErrorAction SilentlyContinue |
            ForEach-Object { Join-Path $_.FullName 'python.exe' } |
            Sort-Object -Descending
    }

    # PATH lookup is intentionally last so the WindowsApps app-execution alias
    # cannot shadow a real interpreter installed under LocalAppData.
    $pathPython = Get-Command python.exe -ErrorAction SilentlyContinue
    if ($pathPython -and $pathPython.Source) { $candidates += $pathPython.Source }

    foreach ($c in ($candidates | Select-Object -Unique)) {
        if (Test-PythonExecutable $c) { return $c }
    }
    return $null
}
...
$py = Find-Python
if (!$py -and $RequirePython) {
    throw 'Python 3 was requested by this build step but was not found.'
}
...
if ($result.Python) { Write-Host "  Python        : $($result.Python)" }
else { Write-Host '  Python        : not required for this build path' }
