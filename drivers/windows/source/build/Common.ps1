...
function Normalize-LegacyWdkProjectContract(
    [string]$Path,
    [ValidateSet('Driver','StaticLibrary')][string]$ConfigurationType,
    [string]$DriverType='KMDF',
    [string]$DriverTargetPlatform
){
    $projectDriverTarget=ConvertTo-WdkDriverTargetPlatform $DriverTargetPlatform
    $xml=Read-MsBuildProjectXml $Path
    $ns=Get-MsBuildNamespaceManager $xml
    $root=$xml.DocumentElement
    ...
    if(!$defaultProps -or !$cppProps){throw "WDK project is missing Microsoft.Cpp props imports: $Path"}

    $configurationGroups=@($xml.SelectNodes('/msb:Project/msb:PropertyGroup[@Label="Configuration" and @Condition]',$ns))
    if($configurationGroups.Count -eq 0){throw "WDK project has no Configuration property groups to normalize: $Path"}

    # Legacy libusbK projects can place WDK identity inside PropertySheets. Validate
    # that those groups contain only the known WDK fields, then remove them. The
    # canonical values are materialized into every Configuration group below, so
    # modern and legacy inputs converge on one project contract.
    $legacy=@($xml.SelectNodes('/msb:Project/msb:PropertyGroup[@Label="PropertySheets" and @Condition]',$ns) |
        Where-Object{$_.SelectSingleNode('msb:PlatformToolset',$ns) -or $_.SelectSingleNode('msb:ConfigurationType',$ns) -or $_.SelectSingleNode('msb:DriverType',$ns) -or $_.SelectSingleNode('msb:DriverTargetPlatform',$ns)})
    $allowed=@('PlatformToolset','ConfigurationType','DriverType','DriverTargetPlatform')
    foreach($group in $legacy){
        foreach($child in @($group.ChildNodes|Where-Object{$_.NodeType -eq [System.Xml.XmlNodeType]::Element})){
            if($child.LocalName -notin $allowed){throw "Legacy WDK property group contains unsupported '$($child.LocalName)' and cannot be normalized safely: $Path"}
        }
        $key=Get-MsBuildConfigurationKey ([string]$group.Condition)
        if([string]::IsNullOrWhiteSpace($key)){throw "Legacy WDK configuration condition is not understood: $($group.Condition)"}
        $matchingTargets=@($configurationGroups|Where-Object{(Get-MsBuildConfigurationKey ([string]$_.Condition)) -eq $key})
        if($matchingTargets.Count -ne 1){throw "Expected exactly one Configuration property group for '$key' in $Path; found $($matchingTargets.Count)."}
        $root.RemoveChild($group)|Out-Null
    }
    ...
    $values=[ordered]@{
        ...
        DriverTargetPlatform=$projectDriverTarget
    }
    foreach($target in $configurationGroups){
        foreach($name in $values.Keys){
            if([string]::IsNullOrWhiteSpace([string]$values[$name])){continue}
            $node=$target.SelectSingleNode("msb:$name",$ns)
            if(!$node){
                $node=$xml.CreateElement($name,$root.NamespaceURI)
                $target.AppendChild($node)|Out-Null
            }
    ...
    Save-MsBuildProjectXml $xml $Path
    Assert-WdkProjectContract $Path $ConfigurationType $DriverType -DriverTargetPlatform $projectDriverTarget
    Write-Host ("WDK project normalization: PASS ({0}, platform={1})" -f (Split-Path -Leaf $Path),$projectDriverTarget) -ForegroundColor Green
}
