...
function Replace-RequiredRegex([string]$Text,[string]$Pattern,[string]$Replacement,[string]$Label){
    ...
    if($regexMatches.Count -ne 1){throw "Required regex patch count for '$Label' is $($regexMatches.Count), expected 1."}
...
function Assert-WdkProjectContract(
    ...
    foreach($projectConfig in $projectConfigs){
        ...
        $matchingGroups=@($configGroups|Where-Object{(Get-MsBuildConfigurationKey ([string]$_.Condition)) -eq $key})
        if($matchingGroups.Count -ne 1){throw "WDK project must have exactly one Configuration property group for '$key': $Path"}
