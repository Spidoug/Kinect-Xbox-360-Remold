...
The builder compiles the current source and recreates `drivers\windows\binaries\`. That generated directory is not committed to the repository.

V1 build launchers are UTF-8 **without BOM** so `cmd.exe` reads the first `@echo` correctly. During preparation of the pinned Microsoft Media Foundation virtual-camera sample, the builder pins `NOMINMAX` and the generated C++ source also protects `std::min/std::max` from the Win32 `min/max` macros.
