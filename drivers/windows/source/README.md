...
Use Windows 11 x64 with a current Visual Studio/MSVC x64 C++ toolchain, matching Windows SDK + WDK, and PowerShell 5.1+. The V1 builder is toolchain-discovered rather than hardcoded to one Visual Studio marketing release; the August 2026 validation log successfully detected Visual Studio 18 / `v145` with WDK `10.0.28000.0`.
