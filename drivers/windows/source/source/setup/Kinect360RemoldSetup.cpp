...
constexpr DWORD kSpapiNoSuchDevinst = 0xE000020Bu;

bool IsTransientDevnodeError(DWORD error) {
    return error == ERROR_NO_SUCH_DEVINST ||
           error == ERROR_NOT_FOUND ||
           error == kSpapiNoSuchDevinst;
}

void RequestSynchronousPnPRescan() {
    DEVINST root = 0;
    if (CM_Locate_DevNodeW(&root, nullptr, CM_LOCATE_DEVNODE_NORMAL) == CR_SUCCESS) {
        (void)CM_Reenumerate_DevNode(root, CM_REENUMERATE_SYNCHRONOUS);
    }
}

bool HardwareIdReadyOnService(const std::wstring& hardwareId,
                              const std::wstring& requiredService,
                              unsigned minimumInstances,
                              unsigned* presentInstances = nullptr) {
    if (presentInstances) *presentInstances = 0;
    ...
    for (DWORD index = 0;; ++index) {
        ...
        if (!SetupDiEnumDeviceInfo(set, index, &data)) {
            if (GetLastError() != ERROR_NO_MORE_ITEMS) allReady = false;
            break;
        }
        ...
        const bool serviceMatches = requiredService.empty() || ContainsInsensitive(service, requiredService);
        if (cr != CR_SUCCESS || (status & DN_STARTED) == 0 || problem != 0 || !serviceMatches) {
            allReady = false;
        }
    }
    SetupDiDestroyDeviceInfoList(set);
    if (presentInstances) *presentInstances = found;
    return found >= minimumInstances && allReady;
}
...
bool IsTargetKinectFunction(const std::wstring& upperId, std::wstring* canonical) {
    const auto matches = [&](uint16_t productId) {
    ...
    for (const uint16_t productId : Kinect360RemoldHardware::kAudioProductIds) {
        if (matches(productId)) return true;
    }
    for (const uint16_t productId : Kinect360RemoldHardware::kCameraProductIds) {
        if (matches(productId)) return true;
    }
    for (const uint16_t productId : Kinect360RemoldHardware::kMotorProductIds) {
        if (matches(productId)) return true;
    }
    for (const uint16_t productId : Kinect360RemoldHardware::kControllerHubProductIds) {
        if (matches(productId)) return true;
    }
    // The post-firmware UAC runtime is composite. Preserve MI_00/MI_01/MI_02
    // in the canonical probe identity and accept both Microsoft 1.8 runtime
    // product IDs (02BB and 02C3). This lets the installer follow the actual
    // UACFirmware 01.02.709.00 topology without confusing control with audio.
    for (const wchar_t* runtimeControl : Kinect360RemoldHardware::kAudioRuntimeControlHardwareIds) {
        if (upperId.rfind(runtimeControl, 0) == 0) {
            if (canonical) *canonical = runtimeControl;
            return true;
        }
    }
    for (const wchar_t* runtimeSecurity : Kinect360RemoldHardware::kAudioRuntimeSecurityHardwareIds) {
        if (upperId.rfind(runtimeSecurity, 0) == 0) {
            if (canonical) *canonical = runtimeSecurity;
            return true;
        }
    }
    for (const wchar_t* runtimeCapture : Kinect360RemoldHardware::kAudioRuntimeCaptureHardwareIds) {
        if (upperId.rfind(runtimeCapture, 0) == 0) {
            if (canonical) *canonical = runtimeCapture;
            return true;
        }
    }
    return false;
...
DWORD BindHardwareId(const std::wstring& hardwareId, const std::wstring& infPath, bool* reboot) {
    ...
    // A Kinect USB child can be removed and recreated by PnP while WinUSB is
    // being applied. Kinect 1473 camera 02AE is particularly prone to this:
    // SetupAPI can finish configuring WinUSB, then return 0xE000020B because
    // the superseded devnode vanished during the installation-triggered restart.
    // Treat that as a transient re-enumeration, not an immediate hard failure.
    //
    // Capture the number of present instances before the bind so a multi-Kinect
    // installation is not declared healthy merely because one of several
    // sensors temporarily disappeared during the driver update.
    unsigned expectedInstances = 0;
    (void)HardwareIdReadyOnService(hardwareId, L"", 1, &expectedInstances);
    if (expectedInstances == 0) expectedInstances = 1;

    DWORD lastError = ERROR_GEN_FAILURE;
    bool anyReboot = false;
    constexpr unsigned kBindAttempts = 6;
    constexpr unsigned kSettlePolls = 30;

    for (unsigned attempt = 1; attempt <= kBindAttempts; ++attempt) {
        // Require the physical function to be present immediately before each
        // update attempt. A global rescan is deliberately done between retries
        // rather than manufacturing a non-present devnode.
        present = false;
        e = FindPresentHardwareId(hardwareId, &present);
        if (e != ERROR_SUCCESS && !IsTransientDevnodeError(e)) return e;
        if (!present) {
            RequestSynchronousPnPRescan();
            Sleep(500);
            lastError = ERROR_NO_SUCH_DEVINST;
            continue;
        }
        ...
        if (needsReboot) anyReboot = true;

        if (!installed) {
            if (e == ERROR_SUCCESS) e = ERROR_GEN_FAILURE;
            lastError = e;
            if (!IsTransientDevnodeError(e)) {
                std::fwprintf(stderr,
            ...
            std::fwprintf(stderr,
                L"TRANSIENT_PNP hardware-id=%ls error=0x%08lX attempt=%u/%u; waiting for the replacement devnode.\n",
                hardwareId.c_str(), e, attempt, kBindAttempts);
        } else {
            lastError = ERROR_NOT_READY;
            std::wprintf(
                L"BIND_APPLIED hardware-id=%ls attempt=%u/%u; waiting for all expected instances to return STARTED on WinUSB.\n",
                hardwareId.c_str(), attempt, kBindAttempts);
        }

        // Whether SetupAPI returned TRUE or hit the 0xE000020B removal race,
        // do not declare success until every instance that was present before
        // the bind has returned STARTED on WinUSB. This prevents a two-Kinect
        // machine from looking healthy while one camera is still re-enumerating.
        RequestSynchronousPnPRescan();
        for (unsigned poll = 0; poll < kSettlePolls; ++poll) {
            Sleep(300);
            unsigned presentNow = 0;
            if (HardwareIdReadyOnService(hardwareId, L"WINUSB", expectedInstances, &presentNow)) {
                if (reboot) *reboot = anyReboot;
                std::wprintf(
                    L"INSTALLED hardware-id=%ls method=UpdateDriverForPlugAndPlayDevicesW prestaged=1 "
                    L"recovered-after-reenumeration=%u attempts=%u instances=%u\n",
    ...
    std::fwprintf(stderr,
        L"UpdateDriverForPlugAndPlayDevicesW exhausted transient PnP retries for hardware-id=%ls from INF=%ls. "
        L"Last error=0x%08lX. See %%WINDIR%%\\INF\\setupapi.dev.log.\n",
        hardwareId.c_str(), infPath.c_str(), lastError);
    return lastError;
}
...
DWORD ProbeDeviceStatus(const std::wstring& hardwareId) {
    ...
    if (foundCount != 0) {
        std::wprintf(L"SUMMARY hardware-id=%ls instances=%u ready=%u\n",
                     hardwareId.c_str(), foundCount, readyCount);
        return readyCount == foundCount ? ERROR_SUCCESS : ERROR_NOT_READY;
    }
...
DWORD CycleDevicesByHardwareId(const std::wstring& hardwareId, unsigned* cycled) {
    if (cycled) *cycled = 0;

    // Force a real PnP disable/enable cycle. This is intentionally separate
    // from RestartDevicesByHardwareId(): a healthy Kinect 1473 parent hub can
    // remain STARTED while one of its internal downstream ports (most notably
    // the 02AE camera child) fails to enumerate. DICS_PROPCHANGE is too soft
    // for that condition; cycling the hub rebuilds its two-port child tree.
    unsigned disabled = 0;
    const DWORD disableError = ChangeDeviceStateByHardwareId(hardwareId, DICS_DISABLE, &disabled);
    if (disableError != ERROR_SUCCESS || disabled == 0)
        return disableError != ERROR_SUCCESS ? disableError : ERROR_GEN_FAILURE;
...
DWORD EnsureRoot(const std::wstring& hardwareId, const std::wstring& infPath, const std::wstring& friendlyName, bool* reboot) {
    // ROOT\Kinect360RemoldDevice is a software-enumerated product container. Its
    // INF intentionally has no PnP function driver (Service Name = <none>); it
    // installs the Win32 broker service. Therefore DN_STARTED/Service=WINUSB is
    // NOT a valid readiness criterion for this devnode. Configure it once and
    // accept the device when SetupAPI succeeds and the devnode remains present
    // without a ConfigManager problem code.
    bool present = false;
    DWORD e = FindPresentHardwareId(hardwareId, &present);
    if (e != ERROR_SUCCESS) return e;
    if (!present) {
    ...
    SetLastError(ERROR_SUCCESS);
    const BOOL installed = UpdateDriverForPlugAndPlayDevicesW(
        nullptr, hardwareId.c_str(), infPath.c_str(), INSTALLFLAG_FORCE, &needsReboot);
    if (!installed) {
        e = GetLastError();
        return e == ERROR_SUCCESS ? ERROR_GEN_FAILURE : e;
    }
    ...
    for (unsigned poll = 0; poll < 20; ++poll) {
        ...
        for (DWORD index = 0;; ++index) {
            ...
            const CONFIGRET cr = CM_Get_DevNode_Status(&status, &problem, data.DevInst, 0);
            if (cr != CR_SUCCESS || problem != 0) healthy = false;
        }
        SetupDiDestroyDeviceInfoList(set);
        if (found && healthy) {
            std::wprintf(L"INSTALLED_ROOT hardware-id=%ls method=UpdateDriverForPlugAndPlayDevicesW configured=1\n", hardwareId.c_str());
            return ERROR_SUCCESS;
        }
    }
    if (needsReboot) {
...
void Usage() {
    std::fwprintf(stderr,
        ...
        L"  Kinect360RemoldSetup cycle <hardware-id>\n");
}
}
...
int wmain(int argc, wchar_t** argv) {
    ...
    } else if (_wcsicmp(cmd.c_str(), L"cycle") == 0) {
        if (argc != 3) { Usage(); return 2; }
        unsigned cycled = 0;
        e = CycleDevicesByHardwareId(hardwareId, &cycled);
        if (e == ERROR_SUCCESS) std::wprintf(L"PnP-cycled %u matching device(s).\n", cycled);
    } else {
        Usage(); return 2;
    }
    if (e != ERROR_SUCCESS) {
    ...
    if (reboot && _wcsicmp(cmd.c_str(), L"ensure-root") == 0) {
        std::wprintf(L"REBOOT_REQUIRED: the root device/service update cannot be completed safely in this boot.\n");
        return 5;
    }
