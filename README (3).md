...
The NUI Audio boot identity receives Microsoft UACFirmware through libusb. After re-enumeration as `045e:02bb`, ALSA owns the four-channel USB Audio capture endpoint. The Remold audio service fans that stream out to microphone and acoustic consumers.

The firmware image is not committed to the repository. `scripts/fetch-uac-firmware.sh` obtains/extracts the pinned Microsoft image when required.
