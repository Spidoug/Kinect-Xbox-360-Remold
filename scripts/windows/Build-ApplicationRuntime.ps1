...
try {
    ...
    # All four SynKinect applications use the same Java/Processing runtime.
    # Analyze exactly one representative application instead of repeating jdeps four times.
    $preferredJar = Join-Path $LibRoot 'SynKinect3DScanner.jar'
    if (Test-Path -LiteralPath $preferredJar -PathType Leaf) {
        $jar = Get-Item -LiteralPath $preferredJar
    } else {
        $jar = Get-ChildItem -LiteralPath $LibRoot -File -Filter 'SynKinect*.jar' |
            Sort-Object Name |
            Select-Object -First 1
    }

    if (!$jar) {
        throw "No SynKinect application JAR was found under $LibRoot"
    }
    ...
    Write-Host 'The four SynKinect launchers can share this single runtime.' -ForegroundColor Green
    exit 0
}
catch {
